...
// Saves the lotDocID and redirects the user to input.html
...
// Generates and populates lot cards with the information below
function populateInput() {
  ...
  // Populates the lot card by lotID and orders by timestamp in descending order with a limit of 1
  // in order to provide the latest lot infromation update to users
  db.collection("input")
    ...
    .then((allInput) => {
      ...
      input.forEach((doc) => {
        ...
        inputCard.querySelector(
          ".vehicle"
        ).innerHTML = `Types of Vehicle: ${vehicle}`;
